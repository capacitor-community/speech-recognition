...
@NativePlugin(
    ...
    requestCodes = {
        SpeechRecognition.REQUEST_CODE_SPEECH
    }
)
public class SpeechRecognition extends Plugin implements Constants {
    ...
    public void hasPermission(PluginCall call) {
        call.success(new JSObject().put("permission", hasAudioPermissions(RECORD_AUDIO_PERMISSION)));
    }
    ...
    private class SpeechRecognitionListener implements RecognitionListener {
        ...
        public void onResults(Bundle results) {
            ...
            try {
                ...
                if (this.call != null) {
                    this.call.success(new JSObject().put("status", "success").put("matches", jsArray));
                }
            } catch (Exception ex) {
                this.call.success(new JSObject().put("status", "error").put("message", ex.getMessage()));
            }
        }
